...
export async function getSubscribeEnquiryTableData() {
  // Fake delay
  ...
  try {
   const response = await fetch(
        process.env.NEXT_PUBLIC_ADMIN_API_URL + "api/subscribeenquiry",
        {
          next: { revalidate: 60 }
        }
      );
...
export const updateSubscribeEnquiryAPI = async (id,subscribeenquiry) => {
  // const token = localStorage.getItem("token"); // 🔹 Retrieve token
