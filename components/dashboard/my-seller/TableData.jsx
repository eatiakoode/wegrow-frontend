...
import { getSellerTableData,deleteSellerAPI } from "@/api/seller";
...
const TableData = () => {
   const [sellerList, setSellerList] = useState([]);
    ...
    const deleteSeller = async (id) => {
        ...
        try {
          const data = await deleteSellerAPI(id); // 🔹 Call the API function
          
          // alert(data.message);
          toast.success(data.message);
          setSellerList((prevSellerList) => prevSellerList.filter((seller) => seller._id !== id));
  ...
  let tbodyContent = sellerList?.map((item) => (
    <tr key={item._id}>
      <td scope="row">
      ...
      <td>
        <ul className="view_edit_delete_list mb0">
          <li
            ...
            title="Edit"
          >
            <button  onClick={() => router.push(`/cmswegrow/edit-seller/${item._id}`)}>
          ...
          <li
            ...
            title="Delete"
          >
            <a href="#"  onClick={() => deleteSeller(item._id)}>
